...
impl Error {
    ...
    pub fn new(descr: &'static str, detail: Option<String>, share_num: Option<u8>) -> Error {
        Error {
            descr: descr,
            detail: detail,
            share_num: share_num,
...
pub fn other_io_err(descr: &'static str, detail: Option<String>, share_num: Option<u8>) -> io::Error {
    convert::From::from(Error::new(descr, detail, share_num))
}

/// maps a `ParseIntError` to an `io::Error`
pub fn pie2io(p: num::ParseIntError) -> io::Error {
    convert::From::from(Error::new("Integer parsing error", Some(p.to_string()), None))
}
...
mod tests_custom_err {
    ...
    fn test_custom_error() {
        ...
        let ewd = custom_error::Error::new(desc, Some(detail.to_string()), None);
        ...
        let ewod = custom_error::Error::new(desc, None, None);
