...
pub enum Reproducibility {
    ...
    /// the system's random number generator (`ring::rand::SystemRandom`)
    /// to produce entropy.
    Probabilistic,
    ...
    ///
    /// ## Warning
    /// Do not use this variant unless you are sure of what you are doing*
    WithEntropy(Vec<u8>),
}
...
impl Reproducibility {
    ...
    pub fn probabilistic() -> Self {
        Reproducibility::Probabilistic
    }

    /// Shares will be produced in a deterministic way, using
    /// the given seed for the internal random number generator used to
    /// harvest entropy.
    pub fn deterministic(seed: Vec<u8>) -> Self {
...
// TODO: Add max length ?
static DEFAULT_PADDING_LEN: usize = 512; // r
static MIN_PADDING_LEN: usize = 128; // r min
static DEFAULT_HASH_LEN: usize = 256; // s
static MIN_HASH_LEN: usize = 128; // s min

impl Default for SS1 {
    fn default() -> Self {
        Self::new(DEFAULT_PADDING_LEN, DEFAULT_HASH_LEN).unwrap()
    }
}

impl SS1 {
    /// Constructs a new sharing scheme
    pub fn new(padding_len: usize, hash_len: usize) -> Result<Self> {
        if padding_len < MIN_PADDING_LEN || hash_len < MIN_HASH_LEN {
            bail!(ErrorKind::InvalidSS1Parameters(padding_len, hash_len,));
        }
    ...
    pub fn split_secret(
        ...
        let padding = self.generate_padding(reproducibility, secret, metadata)?;

        let mut vol_hash = VOLHash::new(&SHA256);
        vol_hash.process(&[0]);
        vol_hash.process(&[threshold, shares_count]);
        vol_hash.process(secret);
        vol_hash.process(&padding);

        let random_bytes_len = random_bytes_count(threshold, secret.len() + self.padding_len);
        let total_hash_len = self.hash_len + random_bytes_len;
        let mut full_hash = vec![0; total_hash_len];

        vol_hash.finish(&mut full_hash);
        let (hash, random_bytes) = full_hash.split_at(self.hash_len);

        let underlying = ThSS::new(Box::new(FixedRandom::new(random_bytes.to_vec())));

        let message = [secret, &padding].concat();
    ...
    fn generate_padding(
        ...
        match reproducibility {
            Reproducibility::Probabilistic => {
                ...
                let mut result = vec![0u8; self.padding_len];
                rng.fill(&mut result)
                    .chain_err(|| ErrorKind::CannotGenerateRandomNumbers)?;
                Ok(result)
            }
            Reproducibility::Deterministic(preseed) => {
    ...
    pub fn recover_secret(&self, shares: &[Share]) -> Result<(Vec<u8>, Option<MetaData>)> {
        ...
        let secret_len = secret.len() - self.padding_len;
        let padding = secret.split_off(secret_len);
        // `secret` nows holds the secret

        let sub_scheme = Self::new(self.padding_len, self.hash_len)?;

        let test_shares = sub_scheme.split_secret(
            shares[0].threshold,
            shares[0].shares_count,
            &secret,
            Reproducibility::WithEntropy(padding.to_vec()),
            &metadata,
        )?;
